...
import type { DiskData, BatchInfo } from '../../../models/monitor.model';

interface DiskAnalysisProps {
    serverIp: string;
}

// --- CONSTANTE DE RUTAS CRÍTICAS ---
const CRITICAL_PATHS = ['/backup', '/backup3', '/opt', '/db2logs'];

// --- HELPERS DE CONVERSIÓN DE TAMAÑO ---
...
const DiskAnalysis: React.FC<DiskAnalysisProps> = ({ serverIp }) => {
    ...
    const showBackupsOnly = () => {
        const backups = allMounts.filter(m =>
            CRITICAL_PATHS.some(path => m === path || m.startsWith(path + '/'))
        );
        setSelectedMounts(backups);
    };
    ...
    const exportToPDF = () => {
        ...
        const pdfData = filteredA.map(diskA => {
            ...
            const percentDiffDisplay = usageDiffPercent > 0 ? `+${usageDiffPercent}%` :
                usageDiffPercent < 0 ? `${usageDiffPercent}%` : `0%`;
        ...
        autoTable(doc, {
            startY: 40,
            head: [
                ['Montaje', 'Tamaño', `Uso % (A)`, `Uso % (B)`, 'Dif. (Tamaño)', 'Dif. (%)']
            ],
            body: pdfData,
            styles: { fontSize: 8 },
    ...
    const exportToCSV = () => {
        ...
        const header = [
            'Montaje', 'Filesystem', 'Tamaño Total',
            `Usado (A)`, `Usado (B)`,
            `Uso % (A)`, `Uso % (B)`,
            'Diferencia %', 'Diferencia (Tamaño)'
        ];
                                ...
                                <div style={{
                                    ...
                                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 10, borderBottom: '1px solid #eee', paddingBottom: 5 }}>
                                        ...
                                        <div style={{ display: 'flex', gap: 5 }}>
                                            ...
                                            <button onClick={() => toggleAll(false)} style={{ fontSize: '0.7rem', color: '#6366f1', background: 'none', border: 'none', cursor: 'pointer' }}>Nada</button>
                                        </div>
                                    </div>

                                    {/* SECCIÓN PRESETS */}
                                    <div style={{ display: 'flex', gap: '8px', marginBottom: '10px', flexWrap: 'wrap' }}>
                                        <button onClick={hideSnaps} style={{ fontSize: '0.7rem', padding: '4px 8px', background: '#e0e7ff', color: '#4338ca', border: 'none', borderRadius: '4px', cursor: 'pointer' }}>No Snaps</button>
                                        <button onClick={showBackupsOnly} style={{ fontSize: '0.7rem', padding: '4px 8px', background: '#fef3c7', color: '#b45309', border: 'none', borderRadius: '4px', cursor: 'pointer' }}>Solo Backups</button>
                                    </div>
